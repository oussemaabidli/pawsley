...
export function ProductForm({ id }: ProductFormProps) {
  ...
  useEffect(() => {
    if (existing) {
      setForm({
        ...
        compare_at_price: existing.compare_at_price != null ? String(existing.compare_at_price) : "",
      ...
      const imgs = [...(existing.product_images ?? [])].sort((a, b) => a.sort_order - b.sort_order);
  ...
  const save = useMutation({
    ...
    onSuccess: (productId) => {
      ...
      if (!id) navigate({ to: "/admin/products/$id", params: { id: productId! } });
    },
        ...
        <section className="rounded border border-border bg-card p-6">
          ...
          <div className="space-y-4">
            ...
            <div className={field}>
              ...
              <Input value={form.colors} onChange={(e) => set("colors", e.target.value)} placeholder="Black, White, Brown" />
            </div>
          </div>
        </section>
      ...
      <div className="space-y-6">
        ...
        <section className="rounded border border-border bg-card p-6">
          ...
          <div className="grid grid-cols-2 gap-2">
            {images.map((img, i) => (
              <div key={i} className="group relative aspect-square overflow-hidden rounded bg-secondary">
          ...
          <div className="mt-2">
            <ImageUpload
              ...
              aspect="aspect-video"
            />
            <p className="mt-1 text-xs text-muted-foreground">Add more images. First image is the main one.</p>
          </div>
        </section>

        {/* Organisation */}
        <section className="rounded border border-border bg-card p-6 space-y-4">
          ...
          <div className="space-y-3 pt-2">
            ...
            ] as { key: keyof ProductData; label: string }[]).map(({ key, label }) => (
