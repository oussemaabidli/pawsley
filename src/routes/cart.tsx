  ...
  const applyCoupon = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from("coupons")
        .select("code,type,value,min_subtotal,max_uses,uses,expires_at,active")
      ...
      if (subtotal < Number(data.min_subtotal ?? 0)) {
        toast.error(`Requires ${formatMoney(data.min_subtotal)} minimum`);
        return;
      }
      const disc =
        data.type === "percent"
          ...
          <div className="mt-8 sm:mt-10 grid gap-8 sm:gap-10 lg:grid-cols-[1fr_360px]">
            <div>
              <ul className="divide-y divide-border">
                {items.map((it) => {
                  const img = [...(it.product.product_images ?? [])].sort(
                    (a, b) => a.sort_order - b.sort_order,
                  )[0]?.url;
                  return (
                    <li
                      key={it.id}
                      className="flex gap-3 sm:gap-4 py-5 sm:py-6"
                    >
                      <Link
                        to="/product/$slug"
              ...
              <Button
                ...
                onClick={() =>
                  navigate({
                    to: "/checkout",
                    search: {
                      code: discount > 0 ? couponCode : undefined,
                    } as never,
                  })
                }
              >
