          ...
          <ul className="mt-10 divide-y divide-border">
            {items.map((it) => {
              ...
              return (
                <li
                  key={it.id}
                  className="flex flex-wrap items-center gap-3 sm:gap-4 py-5 sm:py-6"
                >
                  <Link
                    to="/product/$slug"
                    params={{ slug: p.slug }}
                    className="h-20 w-16 sm:h-24 sm:w-20 shrink-0 overflow-hidden rounded-sm bg-secondary"
                  >
                    {img && (
                      <img
                        src={img}
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => add.mutate({ product_id: p.id })}
                    >
                      Add to bag
                    </Button>
                    <button
