...
function AdminContent() {
    ...
    queryFn: async () =>
      ((
        await supabase
          ...
          .eq("key", "pages")
          .maybeSingle()
      ).data?.value as PagesContent | null) ?? {},
  });
  ...
  useEffect(() => {
    if (navData) setNav(navData);
  }, [navData]);
  useEffect(() => {
    if (pagesData) {
      setPages({
        about: {
          headline: pagesData.about?.headline ?? "Considered by design.",
        },
        contact: {
          headline: pagesData.contact?.headline ?? "Say hello",
          subheadline:
            pagesData.contact?.subheadline ?? "We'd love to hear from you.",
        },
      });
    }
  }, [pagesData]);
  ...
  const saveNav = useMutation({
    mutationFn: async () => {
      const { error } = await supabase
        .from("site_settings")
        .upsert({ key: "navigation", value: nav as any });
      if (error) throw error;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin_nav"] });
  ...
  const savePages = useMutation({
    mutationFn: async () => {
      const { error } = await supabase
        .from("site_settings")
        .upsert({ key: "pages", value: pages as any });
      if (error) throw error;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin_pages"] });
      qc.invalidateQueries({ queryKey: ["site_settings"] });
      toast.success("Pages content saved");
    },
    onError: (e: Error) => toast.error(e.message),
  });

  if (loadingNav || loadingPages)
    ...
    <div className="p-4 sm:p-8 space-y-12">
      <section>
        <div className="flex items-center justify-between mb-6">
          ...
          <Button onClick={() => saveNav.mutate()} disabled={saveNav.isPending}>
            Save Navigation
          </Button>
        </div>
        <div className="space-y-4 max-w-2xl">
          ...
          <Button
            variant="outline"
            className="w-full"
            onClick={() => setNav([...nav, { label: "New Link", to: "/" }])}
          >
      ...
      <section>
        <div className="flex items-center justify-between mb-6">
          ...
          <Button
            onClick={() => savePages.mutate()}
            disabled={savePages.isPending}
          >
            Save Pages
          </Button>
        </div>
        <div className="space-y-8 max-w-2xl">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setPages({
                    ...pages,
                    about: { ...pages.about, headline: e.target.value },
                  })
                }
              />
            </div>
          ...
          <div className="space-y-4 rounded border p-6">
                ...
                onChange={(e) =>
                  setPages({
                    ...
                    contact: {
                      ...
                      subheadline: pages.contact?.subheadline ?? "",
                    },
                  })
                }
              />
            ...
            <div>
              ...
              <Input
                value={pages.contact?.subheadline}
                onChange={(e) =>
                  setPages({
                    ...pages,
                    contact: {
