...
function AdminFaqs() {
  ...
  const { data: faqs = [], isLoading } = useQuery({
    queryKey: ["faqs"],
    queryFn: async () => ((await supabase.from("faqs").select("*").order("sort_order")).data ?? []) as FAQ[],
  });

  const [open, setOpen] = useState(false);
  const [form, setForm] = useState<Partial<FAQ>>({ question: "", answer: "", sort_order: 0, visible: true });
  ...
  const openEdit = (faq: FAQ) => { setForm(faq); setEditingId(faq.id); setOpen(true); };

  return (
    <div className="p-8">
      <div className="flex items-center justify-between">
        <h1 className="font-display text-3xl">Manage FAQs</h1>
        <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) { setEditingId(null); setForm({ question: "", answer: "", sort_order: 0, visible: true }); } }}>
          <DialogTrigger asChild><Button>Add FAQ</Button></DialogTrigger>
          <DialogContent>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-2"><Label>Visible</Label><Switch checked={form.visible} onCheckedChange={(v) => setForm({ ...form, visible: v })} /></div>
              </div>
              <Button onClick={() => saveFaq.mutate()} disabled={saveFaq.isPending} className="w-full">Save</Button>
            </div>
          </DialogContent>
        </Dialog>
      </div>
      <div className="mt-8 rounded-sm border border-border bg-card">
        <Table>
          <TableHeader><TableRow><TableHead>Order</TableHead><TableHead>Question</TableHead><TableHead>Visible</TableHead><TableHead className="w-24"></TableHead></TableRow></TableHeader>
          <TableBody>
            {isLoading ? <TableRow><TableCell colSpan={4} className="text-center py-8">Loading...</TableCell></TableRow> : null}
            {faqs.map((f) => (
              <TableRow key={f.id} className="cursor-pointer hover:bg-muted/50" onClick={() => openEdit(f)}>
                ...
                <TableCell>
                  <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); deleteFaq.mutate(f.id); }}><Trash className="h-4 w-4" /></Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
